...
xe1=xe0+Ts*((ye0+q)*w_ref-uf+(eta1+ur)*cos(thetae1));
ye1=ye0+Ts*(-(xe1+p)*w_ref+(eta1+ur)*sin(thetae1));
...
for (i=0;i<=Hp;i++){
low[2*(Hp+1)+4*i+1]=-0.05;
low[2*(Hp+1)+4*i+2]=-0.05;
low[2*(Hp+1)+4*i+3]=-0.5;
low[2*(Hp+1)+4*i+4]=-0.01;
//low[5*(Hp+1)+5*i+5]=-0.05;
up[2*(Hp+1)+4*i+1]=0.05;
up[2*(Hp+1)+4*i+2]=0.05;
up[2*(Hp+1)+4*i+3]=0.5;
up[2*(Hp+1)+4*i+4]=0.01;
}
