...
sm_mlt(Params[2],Q,Q);
//Q->me[0][0]=(Params[2]*5);
Q->me[1][1]=(Params[2]*0.1);
//Q->me[2][2]=(Params[2]*10);
Q->me[3][3]=(Params[2]*0.001);
//m_output(Q);
...
xbcon=m_get(4,Hp+1);
for(i=0;i<=Hp;i++){
xbcon->me[0][i]=x00[4*i];
xbcon->me[1][i]=x00[4*i+1];
xbcon->me[2][i]=x00[4*i+2];
...
ubcon=m_get(3,Hc+1);
for(i=0;i<=Hc;i++){
ubcon->me[0][i]=u00[3*i];
ubcon->me[1][i]=u00[3*i+1];
...
int kk=0;
double u1,u2,u3=0;
double xe0,ye0,thetae0,eta0, de0=0;
double xe1,ye1,thetae1,eta1, de1=0;
...
xe1=xe0+Ts*(ye0*Curv*qsip+u1);
ye1=ye0+Ts*(-xe1*Curv*qsip+(eta1+ur)*sin(thetae1));
...
for (i=0;i<=Hp;i++){
low[3*(Hp+1)+4*i+1]=-0.05;
low[3*(Hp+1)+4*i+2]=-0.05;
low[3*(Hp+1)+4*i+3]=-0.5;
low[3*(Hp+1)+4*i+4]=-0.01;
//low[5*(Hp+1)+5*i+5]=-0.05;
up[3*(Hp+1)+4*i+1]=0.05;
up[3*(Hp+1)+4*i+2]=0.05;
up[3*(Hp+1)+4*i+3]=0.5;
up[3*(Hp+1)+4*i+4]=0.01;
}
...
//printf("\nk=%d\n",k);
//calccoterms(k);
con[4*k+1]=xbcon->me[0][k];
con[4*k+2]=xbcon->me[1][k];
con[4*k+3]=xbcon->me[2][k];
con[4*k+4]=xbcon->me[3][k];
