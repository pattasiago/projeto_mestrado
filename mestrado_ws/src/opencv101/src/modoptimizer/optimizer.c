...
/*                                 user functions                               */
/* **************************************************************************** */
#ifndef _CONS_INCLUDED_H
#define _CONS_INCLUDED_H
	#include "o8para.h"
	#define  X extern
	#include "o8comm.h"
	#include "o8fint.h"
...
for (i=0;i<=Hp;i++){
	if (VPFMode==2){
		...
		low[1*(Hp+1)+2*i+1]=-0.05;
		low[1*(Hp+1)+2*i+2]=-0.3;
		up[1*(Hp+1)+2*i+1]=0.05;
		up[1*(Hp+1)+2*i+2]=0.3;
		}
}
if (VPFMode==1){
